/*
  https://github.com/IcingTomato/WioTerminal-LoRaWAN-Gateway-Tester
*/
...
size_t SoftwareSerial::write(uint8_t b) {
  ...
  EIC->INTENCLR.reg = EIC_INTENCLR_EXTINT(1 << DigitalPin_To_Interrupt(_receivePin));
  // Write the start bit
  if (inv) reg->reg |= reg_mask;
  else reg->reg &= inv_mask;
  delayMicroseconds(delay);
  // Write each of the 8 bits
  for (uint8_t i = 8; i > 0; --i) {
    // choose bit
    if (b & 1) reg->reg |= reg_mask; // send 1
    else reg->reg &= inv_mask; // send 0
    delayMicroseconds(delay);
    b >>= 1;
  }
  // restore pin to natural state
  if (inv) reg->reg &= inv_mask;
  else reg->reg |= reg_mask;
  EIC->INTENSET.reg = EIC_INTENSET_EXTINT(1 << DigitalPin_To_Interrupt(_receivePin)) ;
  delayMicroseconds(delay);
  return 1;
}
